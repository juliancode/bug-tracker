...
class App extends React.Component {
  ...
  workingOnBug(index, bugId) {
    ajax
      .put(`http://localhost:3001/bugs/${bugId}`)
      .set('Content-Type', 'application/json')
      .send({ status: 1 })
      .end((err, res) => {
        if (err || !res.ok) {
  ...
  deleteBug(index, bugId) {
    ajax
      .delete(`http://localhost:3001/bugs/${bugId}`)
      .set('Content-Type', 'application/json')
      .end((err, res) => {
        if (err || !res.ok) {
          alert(`Error: ${err}`);
          ...
          this.setState(prevState => {
            return {
              bugs: [
                ...prevState.bugs.slice(0, index),
                ...prevState.bugs.slice(index + 1)
              ]
            };
          });
        }
